...
@LastEditTime: 2019-08-19 14:12:19
@Description:
...
2) codon_table:
col0: AA, containing 20 AA and '*' corresponding to the codon
col1: codon, containing 64 codons.
3) tRNA gene copy numbers, download from the GtRNAdb
col0: chr
col1: tRNA
col2: start position
col3: stop position
...
4) start_stop_codon_coordinate file generated by OutputTranscriptInfo.py
col0: transcript id
col1: start position of CDS region in transcript level
col2: stop position of CDS region in transcript level

Notes:
1) The script could output two files: one contains local tAI at each position along transcripts. And the other condations the global tAI.
...
def create_parser_for_tAI():
	...
	parser.add_option("-C","--codon_table",action='store',type="string",dest="codon_table",
			help="Codon table file containing two columns,one is AA, the other is codon")
...
def prepare_datas(tRNA_GCN_file,codon_table_file):
	...
	codonList=pd.read_csv(codon_table_file,sep="\t")
	codonList=codonList['codon'].values
...
def parse_args_for_tAI_calculation():
	...
	(transcriptFiles,codon_table,copy_numbers,outPrefix,trans_file_legend,upLength,downLength)=(options.transcriptFiles.split(","),options.codon_table,
	options.copy_numbers,options.output_prefix,options.trans_file_legend.split(","),options.upstream_codon,options.downstream_codon)
